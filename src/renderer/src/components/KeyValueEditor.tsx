...
export default function KeyValueEditor({ pairs, onChange, showEnabled = true }: Props) {
  ...
  return (
    <div className="space-y-1">
      {/* Header */}
      <div className="flex items-center gap-2 px-1 text-xs text-slate-500 font-medium uppercase tracking-wider">
      ...
      {pairs.map((pair, index) => (
        <div key={pair.id} className="flex items-center gap-2 group">
          ...
          <input
            ...
            className="flex-1 bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
          />
          <input
            type="text"
            placeholder="Value"
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center text-slate-600 hover:text-red-400 hover:bg-slate-800 rounded opacity-0 group-hover:opacity-100 transition-opacity"
            title="Remove"
          >
            &times;
          </button>
      ...
      <button
        onClick={addPair}
        className="text-xs text-slate-500 hover:text-blue-400 px-1 py-1 transition-colors"
      >
        + Add row
      </button>
    </div>
